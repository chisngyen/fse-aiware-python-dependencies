% ============================================================
%  Script thuyết trình (booklet) · MEMRES -> CGAR
%  Sinh từ script.md. Sửa script.md rồi yêu cầu "sync script.tex".
%  Biên dịch: latexmk -pdf script.tex   (hoặc pdflatex script.tex)
% ============================================================
...
% -- Palette mượn từ deck metropolis --
\definecolor{navydeep}{HTML}{23373B}
...
\lhead{\small\color{navydeep}\bfseries MEMRES $\to$ CGAR}
\rhead{\small\color{navydeep}Script thuyết trình (30 phút)}
\cfoot{\small\color{navydeep}Trang \thepage\ / \pageref{LastPage}}

% -- Ghi chú sân khấu (không bắt buộc nói) --
\newcommand{\stagenote}[1]{{\small\itshape\color{gray}#1}}

% -- Danh sách gọn --
\setlist[itemize]{leftmargin=1.5em, itemsep=2pt, topsep=3pt, parsep=1pt}

% -- Heading cho mỗi phần --
\newcommand{\parthead}[2]{%
  \vspace{1.4em}\par
  {\noindent\color{navydeep}\rule{\linewidth}{1.4pt}}\\[0.35em]
  {\bfseries\Large\color{navydeep}#1}\hfill{\normalsize\color{accent}\bfseries #2}\\[-0.35em]
  {\noindent\color{navydeep}\rule{\linewidth}{1.4pt}}
  \par\vspace{0.6em}%
}

% -- Heading cho mỗi slide --
\newcommand{\slidehead}[3]{%
  \vspace{1.05em}\par\noindent
  {\color{accent}\rule[-0.18em]{3pt}{1.05em}}\hspace{0.5em}%
  {\bfseries\large\color{navydeep}Slide #1 \textperiodcentered\ #2}%
  \hfill{\small\color{gray}(#3)}\\[0.25em]%
}
...
\begin{center}
  \vspace*{2.2cm}
  {\small\color{accent}\bfseries MACHINE LEARNING COURSE PROJECT \textperiodcentered\ 2026}\\[1.4em]
  {\Huge\bfseries\color{navydeep} MEMRES $\to$ CGAR}\\[0.6em]
  {\Large\color{navydeep} Script thuyết trình chi tiết}\\[0.4em]
  {\large\color{gray} Agentic Python Dependency Resolution}\\[2.0em]
  {\large Nhóm \textbf{The Fangs}}\\[0.4em]
  {\normalsize Huỳnh Trung Kiệt \textperiodcentered\ Đào Sỹ Duy Minh \textperiodcentered\ Trần Chí Nguyên}\\[0.3em]
  {\small\color{gray} University of Science, VNU-HCM}\\[2.4em]

  \setlength{\fboxsep}{10pt}%
  \colorbox{softgray}{%
    \begin{minipage}{0.82\linewidth}
      \small
      \textbf{\color{navydeep}Tổng thời lượng phần chính (slide 1--31): $\approx$ 30 phút 20 giây.} Backup (B1--B8) chỉ dùng khi Q\&A.\\[0.5em]
      \textbf{Phân bổ:} Mở đầu 1$'$00$''$ \textperiodcentered\ P1 Bài toán 7$'$20$''$ \textperiodcentered\ P2 MEMRES 3$'$40$''$ \textperiodcentered\ P3 CGAR 9$'$00$''$ \textperiodcentered\ P4 Kết quả 6$'$10$''$ \textperiodcentered\ P5 Hạn chế 3$'$10$''$.\\[0.5em]
      {\color{gray}Con số ($\approx$X giây) ở mỗi slide là thời lượng nói mục tiêu. Ghi chú \emph{in xám nghiêng} là tùy chọn, nói hay không đều được.}
    \end{minipage}%
  }
\end{center}
\newpage
...
% ==================== MỞ ĐẦU ====================
\parthead{Mở đầu}{$\approx$ 1$'$00$''$}

\slidehead{1}{Giới thiệu đề tài \& nhóm}{$\approx$35s}
...
\slidehead{2}{Mục lục bài trình bày}{$\approx$25s}
...
% ==================== PHẦN 01 ====================
\parthead{Phần 01 \textperiodcentered\ Bài toán}{$\approx$ 7$'$20$''$}

\slidehead{3}{Phần 01: Bài toán}{$\approx$10s}
Đầu tiên là phần bài toán: bối cảnh, đề bài, và câu hỏi vì sao nó lại khó.

\slidehead{4}{Bối cảnh}{$\approx$75s}
...
\begin{itemize}
  \item Code lẫn cả Python 2 và Python 3.
  \item Wheel không build được trên máy hiện tại.
  \item Hay là file requirements mất nên không biết để cài.
  \item Hoặc là API mà code gọi đã bị xóa ở version mới.
\end{itemize}
...
\slidehead{5}{Bài toán \& yêu cầu đánh giá}{$\approx$85s}
...
Mọi import phải chạy được trong Docker, và phải không có ImportError, không ModuleNotFoundError, không SyntaxError xảy ra. Có mấy hard requirements bắt buộc \stagenote{(optional: nhãn màu đỏ nhé, kiểu nói cũng được không nói cũng được)}

Mọi thao tác đều phải chạy trong Docker. LLM phải là open-source, và dưới 10GB VRAM. Và nhóm em sử dụng model Gemma-2 9B qua Ollama giống paper của ban tổ chức để dễ so sánh. Về chi phí đánh giá, các tool đều dùng chung một ngân sách để công bằng: Docker build tối đa 180 giây, nhiều nhất mười lần build mỗi snippet, số bước suy luận logic ký hiệu là K solve tối đa 50 và mấy bước này không đụng tới Docker, chạy một worker, và tổng khoảng 1800 giây mỗi snippet. Metric chính là pass rate, phụ là thời gian trung bình mỗi snippet.

Những cái yêu cầu trên đều được ban tổ chức cuộc thi yêu cầu.

\slidehead{6}{The Dependency Gap}{$\approx$75s}
Có ba loại khó chính.
\begin{itemize}
  \item Thứ nhất là tên import gây hiểu lầm. Ví dụ trong code viết import cv2, nhưng tên gói trên PyPI lại là opencv-python. Hai cái tên khác hẳn nhau.
  \item Loại thứ hai là API biến mất. Ví dụ from scipy dot misc import imread. Cái imread này chỉ tồn tại tới scipy 1.2, đến scipy 1.3 là nó bị bỏ hẳn.
  \item Loại thứ ba là hiệu ứng domino (dependency hell), tức là khi buộc phải chọn scipy từ 1.2 trở xuống, thì kéo theo Python phải từ 3.7 trở xuống, rồi lại kéo theo numpy phải là bản cũ. Cả một chuỗi ràng buộc nối đuôi nhau.
\end{itemize}
Vì vậy dẫn đến phải tìm tổ hợp trong không gian siêu bự.

\slidehead{7}{Bùng nổ tổ hợp}{$\approx$65s}
...
\slidehead{8}{Datasets}{$\approx$75s}
...
\begin{itemize}
  ...
  \item Bộ thứ hai là GitChameleon, đây là bộ out-of-distribution. Gồm 328 snippet Python, mỗi cái kèm unit test thực thi; còn version ground-truth thì được giấu đi, resolver không được thấy. Nó xoáy vào breaking changes giữa các version, tức là bạn phải dùng đúng API của đúng bản thư viện. Tụi em đã re-frame nó từ GitChameleon 2.0 để đo tính generalization của phương pháp cũng như là
\end{itemize}
\stagenote{(note, không đọc: nguồn số closed-model ở slide 24 = GitChameleon 2.0, arXiv 2507.12367, KHÔNG phải bản v1 116 bài)}

\slidehead{9}{Related Work: 3 hướng}{$\approx$75s}
Thì trước đây có ba hướng chính đã được triển khai là:
\begin{itemize}
  \item Hướng thứ nhất là Knowledge Graph. Tức là encode mối quan hệ giữa module và version thành một đồ thị, lấy dữ liệu từ Libraries.io hay PyPI, rồi truy vấn để suy ra dependency. Ưu điểm là cấu trúc rõ ràng và deterministic. Nhưng hạn chế lớn nhất là database lỗi thời rất nhanh.
  \item Hướng thứ hai là regex và log-parsing. Người ta Parse error log bằng regex để đoán ra dependency đang thiếu, rồi patch vào và build lại. Ưu điểm là nhẹ và bám theo runtime. Nhưng nó brittle (dễ nhạy cảm), chỉ cần log đổi định dạng là parser vỡ.
  \item Hướng thứ ba là LLM kết hợp với RAG. Dùng LLM để sinh ra giả thuyết, RAG để cấp thêm PyPI metadata, Docker build phản hồi lại, rồi lặp đi lặp lại. Ưu điểm là adaptive và tận dụng được kiến thức pretrain. Nhưng nó thử sai 1 cách mù quáng, những cái thất bại không được chuyển thành tri thức để prune không gian. Và đó chính là khoảng trống mà bọn em đề xuất method này. Chính là học từ các lần build thất bại để thu hẹp không gian tìm kiếm.
\end{itemize}

\slidehead{10}{Related Work: bảng tổng hợp}{$\approx$55s}
\begin{itemize}
  \item Nhóm Knowledge Graph có PyEGo ở ICSE 2022 và ReadPyE ở TSE 2024.
  \item Nhóm log-parsing có PyDFix ở ISSTA 2021. Nhóm LLM đóng có GPT-4o và o1 năm 2024, rồi GPT-4.1 cộng RAG năm 2025.
  \item Nhóm LLM có PLLM (của ban tổ chức). Và gần nhất là SMT-LLM (co-competitor) ở FSE 2026, kết hợp Z3 với LLM. Nhóm Knowledge Graph chạm ngưỡng trên ở khoảng 47\% trên HG2.9K. Rồi PLLM mở ra cái hướng mới, vượt lên khoảng 55\%.
\end{itemize}

% ==================== PHẦN 02 ====================
\parthead{Phần 02 \textperiodcentered\ MEMRES}{$\approx$ 3$'$40$''$}

\slidehead{11}{Phần 02: MEMRES}{$\approx$10s}
Phần hai: MEMRES, phương pháp nền tảng của nhóm, dựa trên ý tưởng memory-augmented retry, tức là retry có bộ nhớ.

\slidehead{12}{FSE AIWARE Winner}{$\approx$10s}
...
\slidehead{13}{MEMRES: Lookup-First, LLM-Last}{$\approx$95s}
...
\slidehead{14}{Từ MEMRES đến CGAR: vá 3 lỗ hổng}{$\approx$115s}
...
\slidehead{17}{Multi-Agent Loop}{$\approx$110s}
Với một vòng lặp chính thì bước một là Planner. Planner sẽ gọi query\_pypi để lấy dữ liệu package, gọi wheel\_filter để lọc bản có wheel, gọi consult\_llm khi cần hỏi model, rồi gọi solve để ra một assignment version, tức là gán mỗi package một phiên bản cụ thể. Bước hai là Executor. (Lưu ý Executor là tool). Nó build\_docker để dựng môi trường, rồi run\_import để chạy thử import. Xong nó chờ phản hồi là build có OK không. Nếu OK thì Done, thoát vòng lặp. Nếu không OK thì sang bước ba, Analyzer. Analyzer gọi parse\_error để bóc tách lỗi, consult\_llm để hiểu ngữ cảnh, rồi gen\_constraint để sinh ra một ràng buộc. Ràng buộc đó được thêm vào Session Store, và phân loại thành một trong ba nhóm: HARD, SOFT, hoặc UPPER. Nếu vẫn còn kẹt do bí thì mới tới bước bốn, Critic. Critic gọi analyze\_failures để nhìn lại cái chuỗi thất bại, suggest\_strategy để gợi ý hướng khác, ví dụ đổi Python. Điểm mấu chốt là mỗi lần build thất bại không hề phí. Vì nó sinh ra thêm một ràng buộc. Ràng buộc đó khiến Planner ở vòng sau né được cả một vùng version, chứ không chỉ né một điểm. Đây chính là thứ MEMRES cũng như các method trước không có.

\slidehead{18}{Miền giá trị D đến từ đâu}{$\approx$90s}
...
Bước bốn, với mỗi version, giữ lại nếu trường requires\_python thỏa target Python và gói đó có sẵn wheel.

Bước năm, sắp xếp, ưu tiên bản có wheel trước, rồi semver giảm dần, và chỉ lấy tám ứng viên đầu mỗi gói.

\slidehead{19}{Tích lũy ràng buộc: ví dụ}{$\approx$80s}
...
\slidehead{20}{Feedback-Guided Constraint Discovery}{$\approx$100s}
...
\slidehead{21}{Session-scoped Learning}{$\approx$75s}
...
% ==================== PHẦN 04 ====================
\parthead{Phần 04 \textperiodcentered\ Kết quả}{$\approx$ 6$'$10$''$}

\slidehead{22}{Phần 04: Kết quả}{$\approx$10s}
Tiếp theo là đến phần kết quả.

\slidehead{23}{So sánh tổng thể}{$\approx$95s}
...
\slidehead{24}{Phân rã lỗi trên HG2.9K}{$\approx$85s}
...
\slidehead{25}{GitChameleon: Open vs Closed}{$\approx$95s}
...
\slidehead{26}{Tốc độ và Ablation}{$\approx$85s}
...
% ==================== PHẦN 05 ====================
\parthead{Phần 05 \textperiodcentered\ Hạn chế \& Hướng phát triển}{$\approx$ 3$'$10$''$}

\slidehead{27}{Phần 05: Hạn chế \& Kết}{$\approx$8s}
Hạn chế hiện nay của CGAR.

\slidehead{28}{Hard Floor \& Giới hạn kiến trúc}{$\approx$80s}
...
\begin{itemize}
  \item Một, CGAR phụ thuộc PyPI live, nếu PyPI down hoặc rate-limit thì phải fallback về cascade của MEMRES.
  \item Hai, hiện tại mới chỉ chạy được cho Python, chưa generalize sang JS, Rust hay Go.
  \item Ba, LLM Analyzer khá brittle với log lạ nằm ngoài dữ liệu pretrain của Gemma-2 9B.
  \item Bốn, session store mới chỉ single-worker, chưa hỗ trợ học federated cho nhiều người dùng.
\end{itemize}

\slidehead{29}{Hướng phát triển}{$\approx$70s}
Từ những hạn chế đó, nhóm có bốn hướng để phát triển.
\begin{itemize}
  \item Hướng thứ nhất là mở rộng đa ngôn ngữ.
  \item Hướng thứ hai là chia sẻ constraint mà vẫn bảo toàn riêng tư. Ý tưởng là snippet của user A khi fix xong sẽ giúp user B tránh đúng lỗi tương tự, mà không cần lộ code của ai.
  \item Hướng thứ ba là tinh chỉnh khả năng đọc lỗi của LLM. Fine-tune Gemma-2 trên dữ liệu đọc log, để nó phân loại lỗi và sinh constraint chính xác hơn.
  \item Hướng thứ tư là mở rộng đa mô hình. Chạy CGAR trên nhiều backbone mới hiện nay để chứng minh nó tốt cho nhiều loại mô hình.
\end{itemize}

\slidehead{30}{Tài liệu tham khảo}{$\approx$8s}
Phần tài liệu tham khảo chính của nhóm em nằm ở đây.

\slidehead{31}{Video trình bày}{$\approx$10s}
Nhóm cũng có chuẩn bị một video trình bày cách phương pháp hoạt động. Chỉ cần bấm vào thumbnail là xem được.

\slidehead{32}{Cảm ơn \& Hỏi đáp}{$\approx$15s}
Phần trình bày của nhóm đến đây là hết. Cảm ơn thầy đã lắng nghe.

% ==================== BACKUP ====================
\parthead{Backup \textperiodcentered\ chỉ dùng khi Q\&A}{}

\slidehead{B1}{Filter \& Solver: đầu vào đầu ra}{Q\&A}
Đây là hai bước lõi.

CandidateGraphBuilder làm filter: nhận packages và python\_version, gọi endpoint json của PyPI, lọc theo requires\_python và wheel Linux, sort wheel trước mới nhất trước, giữ top 8.
...
Cần nhớ: Ksolve bằng 50 chạy logic tính bằng micro giây, còn Kbuild nhỏ hơn hoặc bằng 10 là Docker thật, 180 giây mỗi lần, nên rất đắt. \stagenote{(optional)}

\slidehead{B2}{Cách các agent giao tiếp}{Q\&A}
Hệ có 4 agent: Planner, Executor, Analyzer, Critic, trao đổi qua 6 luồng message. Mấu chốt: agent không gọi nhau trực tiếp, CGARResolver làm coordinator điều phối, còn ConstraintStore là bộ nhớ chung mang tri thức chéo snippet. Ví dụ: Analyzer ghi add\_upper\_bound cho scipy bản 1.3, sau đó Planner hỏi get\_upper\_bound và nhận lại 1.3.

\slidehead{B3}{Cấu trúc dữ liệu Session Store}{Q\&A}
Bản ghi lỗi là InfeasibleRecord, gồm package, version, python\_version, error\_type HARD hoặc SOFT, error\_signature, confidence 1.0 hoặc 0.8, và count. Có ba store: records, combo\_records và upper\_bounds. Cơ chế: nhận log lỗi, normalize signature để cắt bỏ số dòng, địa chỉ bộ nhớ dạng 0x và đường dẫn, rồi classify\_error. Regex bắt chuỗi cannot import name X from PKG là tự động add\_upper\_bound.

\slidehead{B4}{Cơ chế gọi LLM}{Q\&A}
...
\slidehead{B5}{Build loop \& chống ảo giác}{Q\&A}
Mỗi snippet reset state rồi loop tối đa 10 Docker attempt, pass thì break. Mỗi attempt đi 4 bước: Planner, Executor, Analyzer, Critic. Phần chống ảo giác: mỗi agent bị ràng ra khỏi tập giá trị hợp lệ của nó. Version Planner chọn phải nằm trong candidate graph. Type Analyzer trả phải là loại hợp lệ đã định nghĩa. Action Critic phải là continue, switch\_python hoặc mark\_unfixable. JSON parse fail thì regex salvage.

\slidehead{B6}{Prompt của Planner}{Q\&A}
...
\slidehead{B7}{Prompt của Analyzer}{Q\&A}
Analyzer prompt phân loại lỗi thành HARD, SOFT hoặc UPPER. Luật trọng tâm: gặp cannot import name X from PKG thì gán UPPER và điền upper\_bound bằng bản tốt cuối cùng trước khi X bị gỡ bỏ. Output JSON gồm type, package, upper\_bound và reason.

\slidehead{B8}{Prompt của Critic}{Q\&A}
Critic prompt đề xuất pivot: continue, switch\_python hoặc mark\_unfixable. Luật: SyntaxError lặp dưới Python 3 mà chưa thử 2.7 thì switch\_python sang 2.7; ImportError lặp trên 4 lần thì mark\_unfixable; quá 8 attempt lỗi lẫn lộn cũng mark\_unfixable. Output JSON gồm action, target và reason.
