...
\centering
\href{https://www.hcmus.edu.vn}{\includegraphics[height=2.0cm]{hcmus.png}}
...
\column{0.18\textwidth}
\centering
\href{https://conf.researchr.org/home/fse-2026}{\includegraphics[height=1.6cm]{fse.png}}
\end{columns}
...
\vspace{1.1em}
\centering
\scriptsize University of Science, VNU-HCM\\[0.4em]
{\tiny\color{gray}Logo: HCMUS \href{https://www.hcmus.edu.vn}{\textcolor{mLightBrown}{hcmus.edu.vn}} · FSE 2026 \href{https://conf.researchr.org/home/fse-2026}{\textcolor{mLightBrown}{conf.researchr.org/home/fse-2026}}}
\end{frame}
...
\textit{($+$ 8 Backup slides cho Q\&A: Filter \& CSP I/O, Agent Comm., Session Store, LLM Call, Build Loop, 3 Agent Prompts)}
\end{frame}
...
\begin{tikzpicture}[font=\scriptsize]
  \def\rw{0.6}
  % --- night sky: crescent moon + stars (over the graveyard) ---
  \fill[mLightBrown!8]  (-5.2,3.0) circle (0.8);
  \fill[mLightBrown!30] (-5.2,3.0) circle (0.48);
  \fill[white]          (-5.02,3.13) circle (0.42);
  \foreach \sx/\sy in {-3.7/3.35, -2.4/2.95, -0.8/3.45, 0.7/3.15, 1.9/3.4}
    \fill[navydeep!30] (\sx,\sy) circle (0.028);
  % --- graveyard ground (left side only) ---
  \fill[black!5] (-6.35,-0.08) to[out=2,in=180] (-0.2,0.05) -- (-0.2,-1.15) -- (-6.35,-1.15) -- cycle;
  \draw[black!22, line width=0.8pt] (-6.35,-0.08) to[out=2,in=180] (-0.2,0.05);
  ...
  \foreach \x/\h/\ep in {%
    ...
    \fill[black!8] (\x,-0.04) ellipse [x radius={\rw+0.16}, y radius=0.13];
  ...
  % cluster label
  \draw[decorate, decoration={brace, amplitude=4pt, mirror, raise=2pt}, gray!50, line width=0.7pt]
    (-6.05,-0.02) -- (-0.55,-0.02);
  \node[font=\scriptsize\itshape, gray!75!black] at (-3.3,-0.62) {hàng nghìn gist Python ``chết môi trường''};
  ...
  \draw[-{Stealth[length=4mm]}, mLightBrown, line width=2.4pt, line cap=round]
    (0.0,1.0) .. controls (1.5,1.1) and (2.1,1.62) .. (3.0,1.75);
...
\scriptsize
\textbf{\color{mDarkTeal}Hard requirements}\\[2pt]
...
\column{0.34\textwidth}
\scriptsize
\textbf{\color{mDarkTeal}Evaluation budget} {\tiny\color{gray}(giống nhau mọi tool)}\\[2pt]
\textcolor{mDarkTeal!22}{\rule{\linewidth}{0.5pt}}
\begin{itemize}\setlength{\itemsep}{2pt}\scriptsize
...
\scriptsize
\textbf{\color{mDarkTeal}Metrics}\\[2pt]
\textcolor{mDarkTeal!22}{\rule{\linewidth}{0.5pt}}
\begin{itemize}\setlength{\itemsep}{2pt}\scriptsize
...
\begin{tikzpicture}[font=\scriptsize,
  ...
  % --- rays fanning into the explosion cloud ---
  \foreach \yc in {1.65,0.45,-0.75}{
    \foreach \dy in {0.34,0,-0.34}{
      \pgfmathsetmacro\vy{\yc+\dy}
      \pgfmathsetmacro\ty{\vy*0.4}
      \draw[mLightBrown!35, line width=0.3pt] (3.5,\vy) -- (5.05,\ty);
  }}
  % --- EXPLOSION cloud: ~10^7 candidate environments (navy -> orange, denser to the right) ---
  \foreach \cx in {5.1,5.35,...,8.1}{
    \pgfmathsetmacro\hh{0.3+(\cx-5.1)*0.82}
    \pgfmathtruncatemacro\nn{3+(\cx-5.1)*4.6}
    \pgfmathtruncatemacro\pp{84-(\cx-5.1)*23}
    \foreach \r in {1,...,\nn}{
      \pgfmathsetmacro\yy{-\hh + 2*\hh*(\r-0.5)/\nn}
      \fill[navydeep!\pp!mLightBrown] (\cx,\yy) circle (0.032);
    }
  }
  \node[font=\tiny, navydeep!75] at (5.95,2.55) {$\times\,\sim$\textbf{8} Python};
  % --- the single valid solution (needle in the haystack) ---
  \fill[mLightBrown] (7.05,0.0) circle (0.075);
  \draw[mLightBrown!85!black, line width=0.6pt] (7.05,0.0) circle (0.18);
  \draw[mLightBrown!85!black, line width=0.4pt] (7.05,0.2) -- (7.3,1.2);
  \node[font=\tiny\bfseries, mLightBrown!85!black, anchor=south] at (7.3,1.2) {1 lời giải};
  % --- climax ---
  \node[font=\footnotesize\bfseries, alert] at (6.6,-2.7) {$\approx 10^{7}$ tổ hợp};
\end{tikzpicture}}
...
\centering
\href{https://arxiv.org/abs/2604.16941}{\includegraphics[width=\linewidth]{memres-pipeline.png}}
...
\begin{tikzpicture}[
  ...
  \foreach \y/\n/\gt/\gd/\ft/\fd in {%
      ...
      1.3/2/{Chi phí lỗi cao}/{335s/snippet, ca khó vẫn rất chậm}/{wheel\_filter()}/{chỉ thử bản có wheel},%
  ...
  \node[align=center] at (-0.475,-1.2) {%
    ...
    \textcolor{alert}{\textbf{LLM đoán mò}}$\;\xrightarrow{\;\text{paradigm shift}\;}\;$\textcolor{success!45!black}{\textbf{Multi-Agent + CSP}} với ràng buộc học được};
\end{tikzpicture}
...
\begin{frame}{CGAR: Tổng quan 3 Agent}
    \begin{figure}
        \centering
        \href{https://chatgpt.com/share/6a2a3c91-5640-83ec-8761-25876b30abe3}{\includegraphics[height=0.74\textheight]{cgar-overview.png}}
    \end{figure}
    \vspace{-0.5em}
    \centering\footnotesize\color{mDarkTeal}
...
\vspace{0.4em}
{\centering\footnotesize\color{mDarkTeal}\textbf{Session = 1 repository = nhiều snippets:} ràng buộc học được \emph{chia sẻ} giữa các snippet trong cùng session.\par}

\vspace{0.3em}
\begin{figure}
    \centering
    \href{https://chatgpt.com/share/6a2a3c8b-93f8-83ec-9452-052ef1ef7e87}{\includegraphics[width=0.82\textwidth]{session-learning.png}}
...
\begin{tabular}{@{}c l l l r r r c@{}}
  ...
  \rowcolor{accent!18}
  10& \textbf{CGAR}        & \textbf{(ours)} & \textbf{Multi-Agent + CSP} & \textcolor{success!50!black}{\textbf{87.1\%}} & \textcolor{success!50!black}{\textbf{83.2\%}} & \textbf{22.3s} & \textcolor{success}{\ding{51}} \\
  \bottomrule
\end{tabular}
...
\begin{tabular}{@{}l l r c@{}}
  ...
  \rowcolor{accent!22}
  \textbf{CGAR}       & open + Agent+CSP & \textcolor{success!50!black}{\textbf{83.2\%}} & \textcolor{success}{\ding{51}} \\
  \bottomrule
\end{tabular}
...
\column{0.18\textwidth}
\centering
\href{https://www.hcmus.edu.vn}{\includegraphics[height=1.4cm]{hcmus.png}}
...
\column{0.18\textwidth}
\centering
\href{https://conf.researchr.org/home/fse-2026}{\includegraphics[height=1.1cm]{fse.png}}
\end{columns}
\end{frame}
