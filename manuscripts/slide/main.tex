...
\begin{tikzpicture}[font=\scriptsize,
  ...
  \node<1->[snip] (s1) at (-4.7,2.5) {import scipy, numpy\\ {\scriptsize\rmfamily target Python $\pi=3.7$}};

  \node<2->[num] at (-6.5,1.4) {2};
  \node<2->[mapb] (s2) at (-4.7,1.4) {\textbf{import $\to$ pip} {\tiny(static map)}\\ \texttt{scipy.misc}$\to$\texttt{scipy}, \texttt{numpy}$\to$\texttt{numpy}};
  ...
  \node<3->[card] (json) at (2.8,1.05)
    {{\bfseries\color{navydeep}PyPI JSON (rút gọn)}\\[1pt]\textcolor{navydeep!30}{\rule{\linewidth}{0.5pt}}\\[2pt]
     ...
     {\ttfamily\tiny\quad 1.2.0 $\to$} {\rmfamily\tiny (imread đã bỏ)}\\
  ...
  \node<5->[card] (dom) at (2.8,-1.6)
    {{\bfseries\color{cUpper!85!black}Domain $D$ (đầu vào solver)}\\[1pt]\textcolor{cUpper!40}{\rule{\linewidth}{0.5pt}}\\[2pt]
     ...
     {\rmfamily\tiny\color{alert}$\approx\!10^{7}$ tổ hợp $\Rightarrow$ $\sim$8 ứng viên/gói}};
...
\scriptsize
\textbf{\color{alert}KHÔNG phải CSP tĩnh.} Ràng buộc thật \emph{ẩn}, chỉ lộ qua build --- verifier hộp đen $f(\mathcal{A})\!\to\!\{\textsf{pass},\,\textsf{typed-error}\}$.\\
{\tiny\color{gray}$C_0=\varnothing$ (không cho trước). Mỗi lỗi Docker $=$ một \emph{counterexample} $\Rightarrow$ agent \textbf{acquire} ràng buộc (\emph{Constraint Acquisition}, CA --- CEGIS-style), \textbf{không} solve một CSP có sẵn.}

\vspace{0.35em}
\textbf{Search state} $\mathcal{S}_t = \langle X, D, C_t \rangle$ --- $C_t$ lớn dần.
