...
\begin{figure}
    \centering
    \includegraphics[width=0.97\textwidth]{boicanh.png}
\end{figure}
...
\column{0.34\textwidth}
\scriptsize
\textbf{\color{alert}Evaluation budget} {\tiny\color{gray}(giống nhau mọi tool)}
\begin{itemize}\setlength{\itemsep}{1pt}\scriptsize
  \item \textbf{180s wall-clock / snippet} \tiny\color{gray}(gồm cả thời gian LLM reasoning + Docker build)
  \item Trong đó: $\le$\textbf{10} vòng Docker build, $K_{\text{solve}}\le$50 CSP {\tiny\color{gray}(logic, no Docker)}
  \item Sequential, 1 worker
  \item \tiny\color{success!50!black}\textit{CGAR thường về đích trong $\le$5 vòng build (tỉa không gian) $\Rightarrow$ dùng \textbf{ít budget hơn} trong cùng trần 180s}
\end{itemize}
...
\begin{itemize}\setlength{\itemsep}{1pt}\scriptsize
  ...
  \item Avg time / snippet \tiny\color{gray}(đo \emph{trong} cùng budget 180s)
\end{itemize}
\end{columns}
\end{frame}
...
\begin{tikzpicture}[
  ...
  \node[tag] at (4.7, 2.30) {scipy $\le$ 1.1};
...
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[
  x={(0.95cm,0.5cm)}, y={(-0.95cm,0.5cm)}, z={(0cm,1.0cm)}, font=\small,
]
  \def\A{4.0}\def\B{2.0}\def\C{2.4}     % packages (x) X versions (y) X python (z)
  \def\nx{6}\def\ny{3}\def\nz{3}
  % 3 mặt
  \fill[navydeep!13] (0,0,0)--(0,\B,0)--(0,\B,\C)--(0,0,\C)--cycle;
  \fill[navydeep!20] (0,0,0)--(\A,0,0)--(\A,0,\C)--(0,0,\C)--cycle;
  \fill[navydeep!28] (0,0,\C)--(\A,0,\C)--(\A,\B,\C)--(0,\B,\C)--cycle;
  % lưới (nhạt, thưa)
  \foreach \i in {0,...,\nx}{\pgfmathsetmacro\px{\i*\A/\nx}
    \draw[navydeep!28,line width=0.3pt] (\px,0,0)--(\px,0,\C) (\px,0,\C)--(\px,\B,\C);}
  \foreach \j in {0,...,\ny}{\pgfmathsetmacro\py{\j*\B/\ny}
    \draw[navydeep!28,line width=0.3pt] (0,\py,0)--(0,\py,\C) (0,\py,\C)--(\A,\py,\C);}
  \foreach \k in {0,...,\nz}{\pgfmathsetmacro\pz{\k*\C/\nz}
    \draw[navydeep!28,line width=0.3pt] (0,0,\pz)--(\A,0,\pz) (0,0,\pz)--(0,\B,\pz);}
  \draw[navydeep,line width=0.8pt]
    (0,0,0)--(\A,0,0)--(\A,0,\C)--(0,0,\C)--cycle
    (0,0,\C)--(\A,0,\C)--(\A,\B,\C)--(0,\B,\C)--cycle
    (0,0,0)--(0,\B,0)--(0,\B,\C) (0,\B,0)--(0,0,0) (\A,0,0)--(\A,0,\C);
  % cube văng ra (mỗi cube = 1 môi trường ứng viên)
  \foreach \px/\py/\pz/\s in {4.5/0.3/2.1/0.42, 5.2/1.0/2.4/0.34, 4.8/1.6/2.5/0.28}{
    \fill[mLightBrown!25] (\px,\py,\pz)--(\px,\py+\s,\pz)--(\px,\py+\s,\pz+\s)--(\px,\py,\pz+\s)--cycle;
    \fill[mLightBrown!40] (\px,\py,\pz)--(\px+\s,\py,\pz)--(\px+\s,\py,\pz+\s)--(\px,\py,\pz+\s)--cycle;
    \fill[mLightBrown!60] (\px,\py,\pz+\s)--(\px+\s,\py,\pz+\s)--(\px+\s,\py+\s,\pz+\s)--(\px,\py+\s,\pz+\s)--cycle;
    \draw[mLightBrown,line width=0.5pt]
      (\px,\py,\pz)--(\px+\s,\py,\pz)--(\px+\s,\py,\pz+\s)--(\px,\py,\pz+\s)--cycle
      (\px,\py,\pz+\s)--(\px+\s,\py,\pz+\s)--(\px+\s,\py+\s,\pz+\s)--(\px,\py+\s,\pz+\s)--cycle
      (\px,\py,\pz)--(\px,\py+\s,\pz)--(\px,\py+\s,\pz+\s);}
  % nhãn trục: mũi tên 2 đầu, đặt LỆCH RA NGOÀI khối (không đè mặt)
  \draw[{Stealth[length=2mm]}-{Stealth[length=2mm]}, navydeep!70, line width=0.6pt] (0,0,-0.5)--(\A,0,-0.5);
  \node[font=\footnotesize, navydeep, anchor=north] at (\A*0.5,0,-0.85) {$\sim$\textbf{500K} packages};
  \draw[{Stealth[length=2mm]}-{Stealth[length=2mm]}, navydeep!70, line width=0.6pt] (-0.5,0,0)--(-0.5,\B,0);
  \node[font=\footnotesize, navydeep, anchor=north east] at (-0.55,\B*0.5,0) {$\sim$\textbf{10} version};
  \draw[{Stealth[length=2mm]}-{Stealth[length=2mm]}, navydeep!70, line width=0.6pt] (0,-0.5,0)--(0,-0.5,\C);
  \node[font=\footnotesize, navydeep, anchor=east] at (0,-0.55,\C*0.5) {$\sim$\textbf{8} Python};
\end{tikzpicture}}
...
\vspace{0.4em}
\begin{center}
\fcolorbox{alert}{alert!10}{\parbox{0.95\linewidth}{\centering
  $\sim$500K $\times$ $\sim$10 $\times$ $\sim$8\\[3pt]
  $\approx$ {\Large\bfseries\color{alert}$10^{7}$} môi trường ứng viên}}
\end{center}
...
\begin{tikzpicture}[font=\scriptsize,
  ...
  % ---- 2. Regex / Log-parsing (cam) ----
  \node[rwcard, draw=mLightBrown!55, fill=mLightBrown!6] (c2) at (0,3.0) {%
    \hspace*{0.62cm}\textbf{\normalsize\color{mLightBrown!85!black}Regex / Log-parsing}\\[3pt]
...
\vspace{0.5em}
{\centering\footnotesize\textcolor{mDarkTeal}{\textbf{$\Rightarrow$ Khoảng trống:} chưa phương pháp nào \emph{học từ thất bại} để thu hẹp không gian tìm kiếm một cách hệ thống.}\par}
\end{frame}
...
\begin{tikzpicture}[
  ...
  gapcard/.style={draw=alert!55, fill=alert!7, rounded corners=5pt, line width=1pt,
                  text width=3.4cm, minimum height=1.55cm, align=center, inner sep=5pt},
  fixcard/.style={draw=success!55!black!55, fill=success!12, rounded corners=5pt, line width=1pt,
                  text width=3.4cm, minimum height=1.15cm, align=center, inner sep=5pt},
  ...
  \def\xa{-4.4}\def\xb{0}\def\xc{4.4}
  % ---- hàng GAP (đỏ) ----
  \node[gapcard] (g1) at (\xa,3.0) {{\color{alert}\xmark\ \textbf{12.8\% bí đường}}\\[2pt]{\tiny API removed / source-only,\\ build kẹt 3 phút}};
  \node[gapcard] (g2) at (\xb,3.0) {{\color{alert}\xmark\ \textbf{Chi phí lỗi cao}}\\[2pt]{\tiny 335s/snippet,\\ ca khó vẫn rất chậm}};
  \node[gapcard] (g3) at (\xc,3.0) {{\color{alert}\xmark\ \textbf{Không tỉa không gian}}\\[2pt]{\tiny Docker error = blackbox,\\ không thành ràng buộc}};
  \node[num] at (\xa-1.5,3.58) {1}; \node[num] at (\xb-1.5,3.58) {2}; \node[num] at (\xc-1.5,3.58) {3};
  % ---- mũi tên "vá" ----
  ...
  % ---- hàng FIX (xanh) ----
  \node[fixcard] (f1) at (\xa,1.0) {{\color{success!45!black}\cmark\ \textbf{Analyzer Agent}}\\{\tiny parse log $\to$ constraint}};
  \node[fixcard] (f2) at (\xb,1.0) {{\color{success!45!black}\cmark\ \textbf{wheel\_filter()}}\\{\tiny chỉ thử bản có wheel}};
  \node[fixcard] (f3) at (\xc,1.0) {{\color{success!45!black}\cmark\ \textbf{3-tier constraints}}\\{\tiny HARD / SOFT / UPPER}};
  % ---- hero CGAR ----
  \node[draw=mDarkTeal, fill=mDarkTeal!12, rounded corners=6pt, line width=1.3pt,
        minimum width=12.6cm, minimum height=1.25cm, align=center] (sol) at (0,-0.95)
    {\textbf{\normalsize\color{mDarkTeal} CGAR: Constraint-Guided Agentic Resolution}\\[3pt]
     \textcolor{alert}{\textbf{LLM đoán mò}} \;$\xrightarrow{\;\text{paradigm shift}\;}$\;
     \textcolor{success!45!black}{\textbf{Multi-Agent + CSP}} với ràng buộc học được};
  \draw[conv] (f1.south) -- ([yshift=1pt]sol.north -| f1);
  \draw[conv] (f2.south) -- ([yshift=1pt]sol.north);
  \draw[conv] (f3.south) -- ([yshift=1pt]sol.north -| f3);
\end{tikzpicture}

\vspace{0.5em}
{\centering\footnotesize Mỗi lỗ hổng MEMRES $\to$ một thành phần CGAR: \textbf{vá có chủ đích}, không refactor mù.\par}
\end{frame}
...
\begin{frame}{CGAR: Tổng quan 3 Agent}
    \begin{figure}
        \centering
        \includegraphics[height=0.80\textheight]{cgar-overview.png}
    \end{figure}
    \vspace{-0.4em}
...
\begin{tikzpicture}[font=\scriptsize,
  ...
  \node[pill] (a1) at (-5.1,2.25) {scipy 1.5};
  ...
  % analyzer -> constraint
  \node[cons] (c1) at (-3.2,1.15) {Analyzer: \textbf{scipy $\le$ 1.2}  {\color{cUpper}[UPPER]}};
  ...
  \node[pill] (a2) at (-5.1,-0.05) {scipy 1.1};
  \node[font=\tiny, gray, anchor=north] at (-5.1,-0.45) {cao nhất $\le$1.2, có wheel};
  ...
  \node[draw=navydeep!50, fill=navydeep!4, rounded corners=6pt, line width=1pt, text width=2.9cm,
    ...
    {{\bfseries\color{navydeep}Constraint Store}\\[2pt]\textcolor{navydeep!30}{\rule{\linewidth}{0.5pt}}\\[4pt]
     {\color{cUpper}$\bullet$}\ scipy $\le$ 1.2 \ {\tiny(UPPER)}\\[6pt]
...
\begin{frame}{CGAR: Online CSP (ràng buộc khám phá dần)}
\begin{columns}[T]
\column{0.50\textwidth}
\scriptsize
\textbf{Formulation:} Tuple $\mathcal{P}_t = \langle X, D, C_t \rangle$.\\
{\tiny\color{gray}$C_t$ \emph{lớn dần}: ràng buộc \textbf{acquire từ Docker verifier} (oracle), không cho trước $\Rightarrow$ \emph{interactive/online CSP}, không phải CSP tĩnh.}
...
\vspace{0.3em}
{\centering\footnotesize\itshape\color{gray} HARD / SOFT / UPPER tỉa cây tìm kiếm $\to$ 1 path khả thi.\par}
\end{columns}
\end{frame}
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{session-learning.png}
...
\textcolor{mDarkTeal}{\textbf{Fairness}: budget = \textbf{180s wall-clock/snippet, giống nhau mọi tool} (gồm cả LLM + build). CGAR chỉ cần $\le$5 vòng build (vs 10) để về đích trong cùng trần $\Rightarrow$ time thấp là do \emph{hội tụ nhanh}, không phải budget khác. Speed headline so trên \textbf{GitChameleon} (mọi tool cùng cấu hình).}
\end{frame}
...
\vspace{0.4em}
\scriptsize \textbf{Insight}: CGAR triệt tiêu \textbf{4 nhóm lỗi}; gần như chỉ còn ImportError (99.7\% phần còn lại).\\
...
\vspace{0.5em}
{\centering\footnotesize\color{mDarkTeal}$\Rightarrow$ Mục tiêu dài hạn: từ bộ giải cho Python trở thành \textbf{``environment archaeologist''} đa năng cho mọi ecosystem.\par}
\end{frame}

\begin{frame}{Tài liệu tham khảo}
\footnotesize
\begin{enumerate}\setlength{\itemsep}{0.45em}
  \item E. Horton, C. Parnin. \textit{Gistable: Evaluating the Executability of Python Code Snippets on GitHub}. ICSME 2018. {\scriptsize\color{gray}(nguồn dataset HG2.9K · arXiv:1808.04919)}
  \item H. Ye, W. Chen, W. Dou, G. Wu, J. Wei. \textit{Knowledge-Based Environment Dependency Inference for Python Programs} (\textbf{PyEGo}). ICSE 2022.
  \item \textit{Revisiting Knowledge-Based Inference of Python Runtime Environments} (\textbf{ReadPyE}). IEEE TSE, vol. 50, 2024.
  \item S. Mukherjee, A. Almanza, C. Rubio-González. \textit{Fixing Dependency Errors for Python Build Reproducibility} (\textbf{PyDFix}). ISSTA 2021.
  \item A. Bartlett, C. C. S. Liem, A. Panichella. \textit{Raiders of the Lost Dependency: Fixing Dependency Conflicts in Python using LLMs} (\textbf{PLLM}). arXiv:2501.16191, 2025.
  \item \textit{GitChameleon 2.0: Evaluating AI Code Generation Against Python Library Version Incompatibilities}. arXiv:2507.12367, 2025.
  \item N. Shinn et al. \textit{Reflexion: Language Agents with Verbal Reinforcement Learning}. NeurIPS 2023.
  \item L. de Moura, N. Bjørner. \textit{Z3: An Efficient SMT Solver}. TACAS 2008.
  \item SMT-LLM. \textit{Z3 SMT + LLM imputation cho dependency resolution}. Entry cùng cuộc thi FSE-AIWare 2026 (chưa công bố; số liệu nhóm tự reproduce trong cùng harness).
\end{enumerate}
\end{frame}
